...
print('Reading table IDs from original_turl_train_tables.csv...')
...
with jsonlines.open(FULL_JSON_FILE) as reader:
    for i, jtable in tqdm(enumerate(reader), total=NTOTAL_TABLES):
        ...
        if len(jtable['tableData']) >= ROW_THRESHOLD \
            ...
            rebuild_table(jtable, mode='pandas').to_csv(output_csv_tables_dir + f'/{jtable["_id"]}')
...
print(f'Completed. Wrote {i} tables.')
