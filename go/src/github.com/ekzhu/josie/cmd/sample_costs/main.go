...
func main() {
	...
	flag.IntVar(&listLengthStep, "cost-list-size-step", 500, "Step size for cost estimation")
...
func sampleReadSetCost(db *sql.DB, pgTableSets, pgTableQueries, pgTableReadSetCostSamples string) {
	// Create sample table
...
func sampleReadListCost(db *sql.DB, pgTableLists, pgTableReadListCostSamples string, minLength, maxLength, step, sampleSizePerStep int) {
	// Create sample table
	_, err := db.Exec(fmt.Sprintf(`DROP TABLE IF EXISTS %s;`,
