...
func main() {
	...
	flag.StringVar(&pgTableQueries, "pg-table-queries", "test_1_queries", "Postgres table for the query sets")
	flag.IntVar(&numQueries, "sampling-num-query", 100, "Number of sets to sample as queries")
	flag.IntVar(&numIntervals, "sampling-num-interval", 10, "Number of stratified sampling intervals for query sizes")
	flag.IntVar(&minQuerySize, "sampling-min-query-size", 1, "Minimum query set size to sample")
	flag.IntVar(&maxQuerySize, "sampling-max-query-size", 10000, "Maximum query set size to sample")
	flag.Parse()
...
func sampleSetsStratified(db *sql.DB, minQuerySize, maxQuerySize, numIntervals, numQueries int) {
	...
	for i := 0; i < numIntervals; i++ {
		...
		// }
		log.Printf("Sample %d sets in size interval [%d, %d)", count, start, end)
