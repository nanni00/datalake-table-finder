...
def worker_result_extractor(inp):
    chunk, (dbname, table_name, dataset, size, blacklist) = inp
    ...
    for (algorithm, mode, (query_id, _, result_ids, algorithm_overlaps)) in tqdm(chunk, leave=False, total=len(chunk), disable=False if os.getpid() % 72 == 0 else True):
        match mode: # problem with previous versions...
            ...
            case '_': mode = mode
        if not result_ids:
            rv.append([query_id, None, algorithm, mode, None, None,                 
                       None, None, None, 
                       None, None, None,
...
def extract_results(test_name, k, num_query_samples, num_cpu, pg_dbname, dataset, size, blacklist):
    assert int(k) > 0
    ...
    logging.info(f'Tokens blacklist: {blacklist}')
    ...
    with mp.Pool(processes=num_cpu) as pool:
        for result_file in os.listdir(results_base_dir):
            ...
            logging.info(f'Total work length: {len(work)}, total workers: {num_cpu}, chunk-size: {chunksize}. Starting extraction...')
            extr_res = pool.map(worker_result_extractor, chunks(work, chunksize, pg_dbname, table_name, dataset, size, blacklist))
