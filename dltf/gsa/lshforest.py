...
class LSHForestGS(AbstractGlobalSearchAlgorithm):
    def __init__(self, mode, blacklist, dlh, string_translators, string_patterns, forest_file, num_perm, l, hash_func, spark_config) -> None:
    ...
    def data_preparation(self):
        ...
        mode, blacklist, string_translators, string_patterns, num_perm, hash_func = self.mode, self.blacklist, self.string_translators, self.string_patterns, self.num_perm, self.hash_func
        forest = MinHashLSHForest(num_perm=self.num_perm, l=self.l)

        def create_minhash(table, valid_columns):
            nonlocal mode, blacklist, string_translators, num_perm, hash_func
            token_set = table_to_tokens(table, valid_columns, mode, 'utf-8', blacklist, string_translators, string_patterns)
    ...
    def query(self, results_file, k, query_ids, **kwargs):
        ...
        for query_id in tqdm(query_ids):
            try:
                ...
                token_set_q = table_to_tokens(content_q, valid_columns_q, self.mode, 'utf-8', self.blacklist, self.string_translators, self.string_patterns)
